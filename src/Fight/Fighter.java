...
public class Fighter extends Form {
    ...
    private void damage(Damage d, boolean blocked) {
        ...
        if (atts[1] < -200 && !invisible) {
            ...
            for (int i = 0; i < 30; i++) {
                Blood.newBlood((int) pos[0], (int) pos[1],
                    Math.toRadians((int) (Math.random() * 360 + 1)), data.getBloodArt(atts[0]));
            }
    ...
    private void noMovement() {//no button pressed
        if (controllable) {
        ...
        if (alife) {
            if (pumpen == 0 && stuntime == 0 && shielddamage >= 0) {
                ...
                switch (move) {
                    ...
                    case 5:
                        moveFast(0);
                        break;//Move Right /Speed
                    case 6:
                        moveFast(1);
                        break;//Move Left/Speed
                    case 7:
                        moveFast(2);
                        break;//Move Up/Speed
                    case 8:
                        moveFast(3);
                        break;//Move Down/Speed
                    case 9:
                        attack(0);
                        break;//Attack horizontaly
                    case 10:
                        attack(1);
                        break;//Attack Up
                    case 11:
                    ...
                    case 13:
                        attack(4);
                        break; //Heavy Attack Up
                    case 14:
                        attack(5);
                        break; //Heavy Attack Down
                    case 15:
                        loadKi();
                        break; //Ki laden
                    case 16:
                    ...
                    case 18:
                        spezialAttack(true);
                        break; //Special Attack
                    case 19:
                        spezialAttack(false);
                        break; //Second Special Attack
                    case 20:
                        pumpen();
                        break;
                    case 21:
        ...
        if (controllable) {
            if (!animation.isRunning()) {
                ...
                if (atts[3] >= kiv) {
                    ...
                    specialbild = specials.spezialAttack(original, atts[0], damage, specialplus,
                        pos, left, spielerid);
                    ...
                    if (original) {
                        ...
                        } else if (atts[0] == FighterData.JANEMBA
                            || atts[0] == FighterData.TRUNKSFUTURE) {
                            if (left) {
                                this.pos[0] -= 250;
                            } else {
                                pos[0] += 250;
        ...
        if (controllable) {
			...
			if(attack==2||attack==5)
			{
				...
				lplus=(int) (lplus/1.3);
                    ...
                    Attack a;
                    a = new KiAttack((int) pos[0] + lplus, (int) pos[1] + hplus,
                        new Damage(damage[2], lplus / 10, 0), lplus / 5, 0, kiattackdesign, 50);
                    if (atts[0] == FighterData.HERCULE) {
                        a = new KiAttack((int) pos[0] + lplus, (int) pos[1] + hplus,
                            new Damage(damage[2], lplus / 10, 0), lplus / 5, 0, 74, 35);
                        a.rotate(15);
                        a.setStein();
        ...
        if (!einfliegen) {
                ...
                if (this.specialfightertime > 0) {
                    if (atts[0] == FighterData.GOKU) {
                        if (left) {
                            ...
                            g.drawImage(GameImages.specialfighter[0][pid],
                                (int) ((pos[0] - dim.width / 2 + dim.width)),
                                (int) ((pos[1] - dim.height / 2)),
                                -dim.width,
                                dim.height, null);
                        } else {
                            g.drawImage(GameImages.specialfighter[0][pid],
                                (int) ((pos[0] - dim.width / 2)), (int) ((pos[1] - dim.height / 2)),
                                dim.width,
                                dim.height, null);
                        }
                    }
                    ...
                    if (left) {
                        ...
                        g.drawImage(GameImages.fighter[atts[0]][pid],
                            (int) ((pos[0] - dim.width / 2 + dim.width)),
                            (int) ((pos[1] - dim.height / 2)),
                            -dim.width,
                            dim.height, null);
            ...
            if (specialplus > 1) {
                ...
                if (auraz > 0) {
                    ...
                    g.drawImage(GameImages.aura[aura], (int) ((pos[0] - dim.width / 2)),
                        (int) ((pos[1] - dim.height / 2)),
                        dim.width,
                        dim.height, null);
                    auraz--;
    ...
    private void paintFlug(Graphics g) {
        ...
        if (left) {
            ...
            g.drawImage(GameImages.fighter[atts[0]][2], x + dim.width, y, -dim.width, dim.height,
                null);
        } else {
