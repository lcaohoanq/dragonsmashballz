...
public class ProfilMenu extends MenuData {
    ...
    public Profil paint(Graphics g, Profil profil, MatchHistory history, float flyw) {
        ...
        if (submenu == 0) {
            ...
            String[] pl = {"Fighter Shop", "Item Shop", "Character Build", "Statistic",
                "Achievements", "Back"};
            ...
            switch (submenu) {
                case 1:
                    paintFighterShop(g, profil);
                    break;
                case 2:
                    paintItemShop(g, profil);
                    break;
                case 3:
                    ...
                    break;
                case 4:
                    paintStatistic(g, profil, history);
                    break;
                case 5:
                    paintAchievements(g);
                    break;
    ...
    private void sortStatistic(Profil p, int sort) {
        ...
        for (int i = 1; i < x.length; i++) {
            for (int j = 0; j < x.length - i; j++) {
                if (x[FighterSelection.fighters[fightid[j]]][sort]
                    < x[FighterSelection.fighters[fightid[j + 1]]][sort]) {
                    temp = fightid[j];
        ...
        if (statisticwahl == 2) {
            ...
            for (int i = 0; i < 8; i++) {
                ...
                if (id < Main.fighteranz) {
                    if (!profil.haveFighter(FighterSelection.fighters[fightid[id]])
                        && !FighterData.kaufbar(
                        FighterSelection.fighters[fightid[id]])) {

                        g.drawImage(GameImages.fighterselection[0], x + 10, y + 20 + i * 50, 50, 40,
                            null);
                    } else {
                        g.drawImage(GameImages.faces[FighterSelection.fighters[fightid[id]]],
                            x + 10, y + 20 + i * 50, null);
                    }
                    g.setColor(Color.WHITE);
                    ...
                    ausgb[4] = "" + stat[fid][5];
                    double pergame =
                        (double) stat[fid][2] / ((double) stat[fid][0] + (double) stat[fid][1]);
    ...
    private void paintItemShop(Graphics g, Profil profil) {
        ...
        for (int i = 0; i < 90; i++) {
            ...
            Item it = items.get(i);
            if (it != null) {
                g.drawImage(GameImages.itemicons[it.getBildid()], x, y, null);
            }
            x += 50;
            if (x >= 950) {
                y += 50;
            ...
            for (int i = 0; i < 10; i++) {
                ...
                if (h * 10 + i < fa) {
                    if (darf) {
                        ...
                        int by = y + h * 55;
                        boolean kannkaufen =
                            FighterData.kaufbar(FighterSelection.fighters[h * 10 + i])
                                || profil.haveFighter(
                        ...
                        if (kannkaufen) {
                            if (mx >= bx && mx <= bx + 80 && my >= by && my <= by + 50) {
                                ...
                                g.setColor(new Color(100, 100, 100));
                                if (profil.getZeni() < FighterData.getPrice(
                                    FighterSelection.fighters[h * 10 + i])) {
                                    g.setColor(new Color(100, 0, 0));
                                }
                            }
                            ...
                            g.drawImage(GameImages.faces[FighterSelection.fighters[h * 10 + i]],
                                bx + 15, by + 5, null);
                        } else {
                            g.setColor(new Color(100, 0, 0));
                            g.fillRoundRect(bx, by, 80, 50, 5, 5);
                            ...
                            g.setColor(Color.BLACK);
                            g.fillRect(bx + 13, by + 3, 56, 46);
                            g.drawImage(GameImages.fighterselection[0], bx + 15, by + 5, 50, 40,
                                null);
                        }
