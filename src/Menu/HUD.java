...
public class HUD {
    ...
    private static final String[] killmessages = {"has killed", "has defeated", "has slain",
        "has beaten", "wiped out", "has annihilated"};
    private static final String[] multikill = {"Doublekill", "Triplekill", "Quadrakill",
        "Pentakill", "Hexakill", "Megakill", "Megakill", "Megakill", "Megakill", "Megakill"};
    ...
    public void paintHUDBar(Graphics g, int x, int y, int spiegeln, int leben, int maxleben, int ki,
        int maxki, int blitze, int shield, int spielerid) {
        ...
        if (fanz <= 4) {
            for (int i = 0; i < fighter.length; i++) {
                if (fighter[i] != null) {
                    ...
                    switch (i) {
                        case 0:
                            paintHUDBar(g, 0, 0, 0, leben, maxleben, ki, maxki, blitze, shield, id);
                            break;
                        case 1:
                            paintHUDBar(g, 600, 0, 1, leben, maxleben, ki, maxki, blitze, shield,
                                id);
                            break;
                        case 2:
                            paintHUDBar(g, 0, 550, 2, leben, maxleben, ki, maxki, blitze, shield,
                                id);
                            break;
                        case 3:
                            paintHUDBar(g, 600, 550, 3, leben, maxleben, ki, maxki, blitze, shield,
                                id);
                            break;
                    }
                }
                ...
                for (int i = 0; i < fighter.length; i++) {
                    if (fighter[i] != null) {
                        if (!fighter[i].isDead()) {
                            ...
                            g.drawImage(GameImages.multisteer[steuer.getIconID()], xpos - 20, ypos,
                                null);
                ...
                if (koteams == 3) {
                    ...
                    for (int i = 0; i < fighter.length; i++) {
                        if (fighter[i] != null) {
                            ...
                            int proz = (int) (((float) fighter[i].getAttributes()[1]
                                / (float) fighter[i].getAttributes()[2]) * 100);
                            health.add(proz);
                        }
                    }
