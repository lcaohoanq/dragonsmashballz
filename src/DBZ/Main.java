...
public class Main extends JFrame {
    ...
    public static String ordnerpfad =
        System.getProperty("user.home") + "/BRollGames/DragonSMASHBall";
    ...
    static String bf = "/resources/Ressourcen/";
    //Display
    ...
    private class Loader implements Runnable {
        ...
        public void run() {
            ...
            setFocusable(true);
            requestFocus();
            System.out.println("Render resources.Images...");
            LOADING = "Loading resources.Images";
            new GameImages(main);
            System.out.println("Load resources.Music & Sounds.. ");
        ...
        try {
            if (h) {
                BufferedImage img = new BufferedImage(16, 16,
                    BufferedImage.TYPE_INT_ARGB); // Changed size
    ...
    public static void setGroesse(boolean full) {
        if (full) {
            ...
            main.setLocation((fullresolution[0] - resolution[0]) / 2,
                (fullresolution[1] - resolution[1]) / 2);
    ...
    private void newFight(Fight fight, String modus) {
        ...
        mapimage = new BufferedImage(stage.getDimension().width, stage.getDimension().height,
    ...
    public void paint(Graphics p) {
        ...
        if (!gameLoaded) {
            ...
            if (menu.isOpen()) {
                ...
                //cheaten verhindern : nur im men� erlaubt
                keys.activateCheat();

                if (!menu.isNetworkFight()) {

                    if (vscreen.isActiv()) {
                        keys.getLastKey(); //verhindern von sofort pause �ffnen danach
                        vscreen.paint(g);
                        if (vscreen.canClose()) {//Fight begin
